...
$("#salvar").on('click', function(event) {
	...
	$.ajax({
		type: "POST",
        contentType: false,
        url : pt_br.absolute_url+"/panel-control/lote/cadastro",
...
$(document).off("click","#tabela_visao tr").on("click","#tabela_visao tr",function(){
	...
	$("#cod_piquete").val($(this).find("input[type=radio]").val());
	$("#input_piquete").val($(this).children('td:eq(1)').text());
...
function loadViewTable() {
	...
	$.ajax({
		type: "GET",
        url : pt_br.absolute_url+"/panel-control/piquete/listar",
        dataType: 'json'
    }).done(function(json){
    	
    	$.each(json,function(i,item){
    		linha = "<tr>";
    		linha +="<td><input type='radio' name='cod_piquete' value="+item.cod_piquete+"></td>";
    		linha += "<td>"+item.piquete+"</td>";
